...
class TestHBNBCommandCreate(unittest.TestCase):
    ...
    def test_create_with_params(self, mock_stdout):
        ...
        with patch('sys.stdin', StringIO('create User name="John_Doe" age=30 height=6.2\n')):
            HBNBCommand().cmdloop()
    ...
    def test_create_with_invalid_params(self, mock_stdout):
        ...
        with patch('sys.stdin', StringIO('create User name="John_Doe" invalid_param="invalid"\n')):
            HBNBCommand().cmdloop()
        output = mock_stdout.getvalue().strip()
        self.assertTrue(len(output) > 0)  # Ensuring an ID is printed
